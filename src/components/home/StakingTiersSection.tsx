...
export function StakingTiersSection() {
  return (
    <section id="chapter-ii" className="bg-obsidian">
      <div className="border-b border-white/10 px-6 py-20 sm:px-10 lg:py-24">
          ...
          <div className="mx-auto w-full max-w-6xl px-6 py-16 sm:px-10 lg:py-20">
            <div className="flex flex-wrap items-baseline justify-between gap-4">
              <div>
                ...
                <h3 className="mt-3 font-display text-4xl text-pearl md:text-5xl">
                  {tier.title}
                </h3>
                <p className="mt-2 text-sm text-mist">{tier.stakeRange}</p>
              </div>
              <span className="font-display text-6xl text-white/[0.06] md:text-8xl">
                ...
                <dl className="mt-5 space-y-4">
                  {tier.metrics.map((m) => (
                    <div key={m.label} className="border-b border-white/5 pb-4 last:border-0">
                      ...
                      {m.note ? (
                        <dd className="text-xs text-mist">{m.note}</dd>
                      ) : null}
                    </div>
                  ))}
                </dl>
